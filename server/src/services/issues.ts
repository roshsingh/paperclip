...
export function issueService(db: Db) {
  ...
  return {
    ...
    checkout: async (id: string, agentId: string, expectedStatuses: string[], checkoutRunId: string | null) => {
      ...
      const now = new Date();
      const activePauseHold = await treeControlSvc.getActivePauseHoldGate(issueBrief.companyId, id);
      if (
        activePauseHold &&
        !(await isTreeHoldInteractionCheckoutAllowed(issueBrief.companyId, checkoutRunId, activePauseHold))
      ) {
      ...
      const dependencyReadiness = await listIssueDependencyReadinessMap(db, issueBrief.companyId, [id]);
